...
void solve(){    
        ...
        cin>>s;
        int n = s.length();
        if(isVowel(s[n-1])==1 and s[n-1]!='y' and s[n-1]!='Y'){
        	cout<<s <<" is ruled by Alice." ndl;
        }else if(isVowel(s[n-1])!=1 and s[n-1]!='y' and s[n-1]!='Y'){
        	cout<< s <<" is ruled by Bob." ndl;
        }else cout<< s << " is ruled by nobody." ndl;
