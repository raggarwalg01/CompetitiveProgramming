...
void solve(){    
    ...
    // do{
        ...
        // dbg(sum);
        // return;
        if(pref[n-1]!=0 and  sum%pref[n-1]==0){
        	cout<< -1*sum/pref[n-1] ndl; return;
        }

        if(k==1){
        	cout<<"IMPOSSIBLE" ndl; return;
        }
        ...
        rep(i,-1e5,1e5){
        	int b = i;
        	if(b + sum ==0 or rest == b*sum) continue;

        	if(  (rest - b*sum)%(b+sum) ==0 ){
        		cout<< b spc (rest - b*sum)/(b+sum) ndl;
        		return;
        	}
        } 
        		cout<<"IMPOSSIBLE" ndl;
