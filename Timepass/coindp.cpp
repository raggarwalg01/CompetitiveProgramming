...
int n , d, u, m;
 int dp[1000][1000];
    int fnc(int idx , int amount ,vector<int> & coins){
        if(amount ==0 ) return 1;
        if(idx < 0 ) return 0;
        if(dp[idx][amount]!=-1) return dp[idx][amount];
        int ways = 0;
        for(int i = 0 ; i<=amount ; i+=coins[idx]){
            ways+=fnc(idx-1 , amount - i , coins);
        }
    ...
    memset(dp,-1,sizeof(dp));
        return fnc(coins.size() -1 , amount , coins);
    }
void solve(){    
        
        cin>>n>>d>>u>>m;
        if(m==3)
        cout<<powermod(n,u-d-1,M)+1;
	    else cout<< powermod(2,u-d+1,M);
        return;
