...
std::vector<vector<int>> v(5001,vector<int>(5001));

void presolve(){    
        
    ll cnt=1;
    for(ll tmp = 0; tmp < 2 *5000 - 1; ++tmp) {
        ll z = (tmp < 5000) ? 0 : tmp -5000 + 1;
        for (ll j = z; j <= tmp - z; ++j) {
            v[j][tmp - j]=cnt;
            cnt++;
        }
    }
...
// int maxx , maxy;
// void fnc(int &arr[maxx][maxy] , int i , int j){
// 	if(i>maxx or j> maxy) return;
// 	int val = 0;
// 	if(i-1>=0){
// 		val = arr[i-1][j];
// 	}
...
// 		val = maxx(arr[i][j-1])
// 	}
// 	arr[i+1] = max(arr[i][j] , arr[i-1][j] , arr[i][j-1]);
// }
...
//        	int a, b,c, d;
//        	cin>>a>>b>>c>>d;
//        	maxx = max(max(maxx,a),c);
//        	maxy = max(max(maxy,b),d);
//        	ques.pb({a,b,c,d});
//        } 
//        int grid1[maxx][maxy];
