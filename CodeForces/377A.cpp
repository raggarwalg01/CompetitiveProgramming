...
ll x[]={1,0,-1,0};
ll y[]={0,1,0,-1};
vector<vector<char>> gp(509,vector<char>(509));
vector<vector<ll>> vis(500,vector<ll>(509,0));
ll curr_sum=0;
void dfs(ll i,ll j,ll n,ll m,ll cnt){
     if(i>=n || i<0 || j>=m || j<0 || gp[i][j]!='.' || vis[i][j]==1 || curr_sum==cnt)
           return;
      vis[i][j]=1;
      dbg(i,j);
      curr_sum++;
      rep(k,0,4)
         dfs(i+x[k],j+y[k],n,m,cnt);
