...
void solve(){    
        ...
        rep(i,0,n){
        	vec.pb({v[i],s[i]=='R'? 1 : -1});
        }
        sort(all(vec),cmp);

      map<int, int> RedColor, BlueColor;
      for(int i=0; i<n; i++){
            if(s[i] == 'R'){
                  RedColor[v[i]]++;
            }else{
      ...
      int check =0;
      for(auto i = RedColor.rbegin(); i!=RedColor.rend(); i++){
            if(RedColor[i->first]+check > (n+1)-(i->fi))
            {
                  cout << "NO" << endl;
                  return;
            }
      ...
      check=0;
      for(auto i = BlueColor.begin(); i!=BlueColor.end(); i++){
            if(BlueColor[i->first]+check > (i->fi)){
                  cout << "NO" << endl;
                  return;
            }
            check+=i->se;
