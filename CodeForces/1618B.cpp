...
void solve(){    
    ...
    if(n==3){
        cout<<'a'<<sstrings[0]<<endl;
        return;
    }

    string myanswer;
    myanswer.push_back(sstrings[0][0]);

    rep(i,0,n-3){
        if(sstrings[i][1]==sstrings[i+1][0]){
            myanswer.push_back(sstrings[i][1]);
        }
        else{
            myanswer.push_back(sstrings[i][1]);
            myanswer.push_back(sstrings[i+1][0]);
        }
    }
    myanswer.push_back(sstrings[n-3][1]);
    while(myanswer.length()<n){
        myanswer.push_back(sstrings[n-3][1]);
    }
    cout<<myanswer <<endl;
