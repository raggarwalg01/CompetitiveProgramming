...
void solve(){    
        ...
        if(mx >= half){
        	cout<< n - mx + 1 ndl;return;
        }else{
        	int needdd = max(0ll,half - mx);
        	// int cando = ceil_div(needdd,mx);
        	int needcopy = 0 ;//ceil_div(needdd,mx);
        	int cnt = mx;
        	while(mx < half ){
        		needcopy+= mx + 1;;
        		mx +=mx;
        	...
        	// mx = half;
        	// needcopy+= half;
        	// cout<< needcopy spc mx  spcend;
        	cout<< n - mx +1 +  needcopy ndl;
        }
