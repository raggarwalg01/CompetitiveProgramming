...
int fans1 = lmax, fans2 = lmax;

int fnc(vi &v, int i, int n, int tans, int done)
{
    if (i == n)
    {
        if (done == tans)
    ...
    if (v[i] % 2 == 0)
    {
        if (can >= v[i])
        {
            ...
            v[i] = 0;
            temp = fnc(v, i + 1, n, tans, done);
            v[i] = storevi;
        }
        temp = min(temp, fnc(v, i + 1, n, tans, done));
    }
    else
    {
        if (can == 0)
