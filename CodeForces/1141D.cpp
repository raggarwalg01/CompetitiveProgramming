...
void solve(){    
  ...
  for(char c='a';c<='z';c++){
    while(mp1[c].size()>0 && mp2[c].size()>0){
      res.pb({mp1[c].back(),mp2[c].back()});
      mp1[c].pop_back();
      mp2[c].pop_back();
    }
  }
  rep(i,0,mp1['?'].size()){
    for(char c='a';c<='z';c++){
      while(mp2[c].size() && mp1['?'].size()){
        res.pb({mp1['?'].back(),mp2[c].back()});
         mp1['?'].pop_back();
         mp2[c].pop_back();
      }
  ...
  rep(i,0,mp2['?'].size()){
    for(char c='a';c<='z';c++){
       while(mp1[c].size() && mp2['?'].size()){
         res.pb({mp1[c].back(),mp2['?'].back()});
         mp1[c].pop_back();
         mp2['?'].pop_back();
      }
    }
