...
void solve(){    
    ...
    rep(i, 0, n)
        cin >> a[i];
    vector<ll> odd,even;  //odd -- -ve and zero
                           // even --- +ve
    rep(i, 0, n){
        if (a[i] <= 0) 
            odd.pb(a[i]);
    ...
    if (n == 5){
        ...
        ans =-1e18;
        if (even.size() >= 5)
            ans = max(ans,even[0]*even[1]*even[2]*even[3]*even[4]);
        if (even.size() >= 3 && odd.size() >= 2)
            ans = max(ans, even[0] * even[1] * even[2] * odd[0] * odd[1]);
        if (even.size() >= 1 && odd.size() >= 4)
            ans = max(ans, even[0] * odd[0] * odd[1] * odd[2] * odd[3]);
    }
    cout << ans ndl; 
