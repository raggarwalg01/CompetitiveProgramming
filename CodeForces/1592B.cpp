...
void solve(){    
        ...
        if(n%2==1){
    			...
    			for(int i=0; i<cnt; i++){
    				if(b[start].fi != v[start].fi || b[end].fi != v[end].fi){
    					cout << "NO" << endl;
    					return;
    				}
    				start--;
...
void solve(){    
        ...
        int mx = imax;
         // x = x - n/2;
        // rep(i,0,n){
        // 	if(b[i].se!=i && b[i].fi !=v[i].fi){
        // 		// int cnt = b[i].se - 0;
        // 		// int cnt2 = n-1 -b[i].se;
        // 		// cerr<< cnt spc cnt2 ndl;
        // 		// int temp = min(cnt , cnt2);
        // 		// mx = max(mx,temp);
        // 		if(i<n/2){
        // 			if(n/2-i <=x){
        // 				NO return;
