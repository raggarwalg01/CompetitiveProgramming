...
void solve(){    
        ...
        rep(i,0,n){
        	...
        	// if(present(v1,{a,b}) || present(v1,{b,a})) continue;
        	// if(v1.present({a,b}))
        	v1.ins({a,b});
        }
        n = v1.sz;
        ...
        int cnt = 0;
        int ele = v[0].fi;
        unordered_map<int , set<int>> hash1,hash2;
        rep(i,1,n){
        	if(v[i].fi != ele and v[i].se != ele){
        		cnt++;
        		hash1[v[i].fi].ins(v[i].se);
        		hash1[v[i].se].ins(v[i].fi);
        	}
        }
        if(cnt==0){
        	YES return;
        ...
        rep(i,1,n){
        	if(v[i].fi != ele and v[i].se != ele){
        		cnt++;
        		hash2[v[i].fi].ins(v[i].se);
        		hash2[v[i].se].ins(v[i].fi);
        	}
        }
        if(cnt==0){
        	YES return;
