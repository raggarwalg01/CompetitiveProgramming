...
void solve(){    
        ...
        if(a>b){
            ...
            rep(i,0,32){
                if((po2[i]&a) == (po2[i]&b)) continue;
                // cout<< po2

                if( (po2[i]&a)==(po2[i]) && ( (po2[i]&b)!= po2[i]) ){
                    cnt += (po2[i]) - ((po2[i]-1) & bin);
                    b+=po2[i];
                    int num = (po2[31]-1) ^ (po2[i]-1);
                    bin = num & bin;
                    // b = b^po2[i];
                }else{
                    // dbg(a,b);
                    ...
                    // cout<<getbinary(a) ndl;
                    // cout<<getbinary(b) ndl;
                    // cndl;
                    // dbg(i,cnt);
                    dbg(i,po2[i],a ,b,cnt, po2[i],po2[i]&a ,po2[i]&b);
                // if(po2[i]&b == po2[i] && po2[i]&a != po2[i]){
                //     cnt++;
                //     a = a|b;
                // // }
                // if((po2[i]&a) ==po2[i] &&  po2[i]&b == po2[i]){
