...
void solve(){    
        ...
        int val = lmax;
        rep(i,0,n){
        	int movecurr = a*(abs(v[i] - capital));
        	ans += movecurr;
        	// int now = (a+b)*(v[i]);
        	sum-=v[i];

			int k = n - i -1 ;
        	int shiftcost = b*abs(v[i]- capital);
        	int shiftcostconquer = a*abs(sum - k*v[i]);
        	int costnormal = a*abs(sum - k*capital);

			// cout<< i spc capital spc movecurr  spc sum ndl;
        	if(costnormal >=shiftcostconquer + shiftcost){
        		ans+= shiftcost;
        		capital = v[i];
        	}else{
        		;
        	...
        	// int shift = b*(sum - k*v[i]);
        	// cout<< i spc k spc capital  spc ans ndl;
        	// cout<< i spc capital spc movecurr  ndl;
        	// cndl;
        	// cout<< i spc ans ndl;
        	// cout<< capital spc movecurr spc costnormal spc shiftcost spc shiftcostconquer ndl;
        	// // cout<<  ndl;
        	// val = min(val , now + shift);
        }
        cout<< ans ndl; return;
