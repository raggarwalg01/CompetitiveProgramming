...
int calculate( int number ){
    ...
    if(done[number]!=0)
        ...
        done[number] = (ans2*ans1)%modnum;
        return done[number];
    }
    // if(done[number]==0){
    //     done[num1] =calculate(num1);
    //     done[num2] = calculate(num2); 
    //     // return  ;
    //     done[number] = (done[num1]*done[num2]);
    // }
    // done[number] = (done[num1]*done[num2]);
    // done
}
...
void solve(){    
        ...
        while(!dq.empty()){
            if(!dq.empty() and (dq.front() <=4)  ){
                ...
                dq.pop_front();
                dq.pb(num/2);
                dq.pb( ceil_div(num,2));
            }
        }
        int numans = 1;
        trav(i,ans){
            numans = numans*i;
            numans = numans %998244353;
        }
        cout<< numans;
