...
bool dist(double x1, double y1, double x2, double y2, double r)
{
    if( sqrt(5) - abs(sqrt((power((x2 - x1), 2)) + (power((y2 - y1), 2))) - r ) >= 0){
    	// cout<<(sqrt((pow((x2 - x1), 2)) + (pow((y2 - y1), 2))) - r ) ndl;
    	return true;
    }
    return false;
...
void solve(){    
    ...
    for(int i=x1;i<=x2;i++){
        for(int j=y1;j<=y2;j++){
            int dis1=((i-a)*(i-a))+((j-b)*(j-b));
            int dis2=((c-i)*(c-i))+((j-d)*(j-d));
            if(dis1==5 && dis2==5){
                Yes
                return;
            }
    ...
    No
    // if(dist(a-c,b-d,0,0, sqrt(5)) || dist(c-a,d-b,0,0,sqrt(5))){
    // 	Yes;
    // }else{
    // 	No;
    // }
