...
#define ins                 insert
#define sz                  size()
#define all(var)            var.begin(), var.endpos()
#define present(c, x)       (c.find(x) != c.endpos())
...
// #ifndef ONLINE_JUDGE
// #define dbg(x...) cerr <<__func__<<" : "<<__LINE__<<" [" << #x << "] = ["; _print(x) ; cerr<< endposl;
// #else
// #define dbg(x...)
// #endposif
...
void solve(){    
	...
	while(startpos < s.size() && s[startpos] == 'a')
	{
		startpos++;
		cnt1++;
	}
	while(endpos >= 0 && s[endpos] == 'a')
	{
		endpos--;
		cnt2++;
	}
