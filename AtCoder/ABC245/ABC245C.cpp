...
int dp[N][2];
int k;
// bool can(vvi vv , int id , int t){

// 	if(id <=0) return true;
// 	if(id >=1){
// 		if( abs(vv[id][t] - vv[id-1][0]) <=k or abs(vv[id][t] - vv[id-1][1])<=k ){
// 			return true;
// 		}else 
// 		return false;
// 	}
...
// 	// dp[id-1][0] = 
// 	// dp[id-1][1] = can(vv,id-1,1);
// 	if( (can(vv,id-1,0) ==1 ) or ( can(vv,id-1,1) ==1 )){
// 		// cout<< id spc t ndl;
// 		check = true;
// 		dp[id][t] = 1;		
// 	}
...
// }
bool check ( vvi & v , int id , int type, int type2){
	if(id==0) return true;
	// if(dp[i][t])

	if( abs(v[id][type] - v[id-1][type2] ) <= k )
		return true;
	return false;
}
bool can(vvi &v , int id , int type){
	
	if(id ==0) return true;
	if(dp[id][type]!=-1) return dp[id][type];

	bool yesno = false;

	if( can(v,id-1,0) and check( v, id ,type,0 )){
		yesno = true;
	}
	if( can(v,id-1,1) and check( v, id ,type,1 )){
		yesno |= true;
	}

	return dp[id][type]= yesno;
...
void solve(){    
        ...
        if(can(vv,n-1,0) or can(vv,n-1,1)){
        	Yes;
        }else No;

        // cout<<dp[n-1][0] spc dp[n-1][1] ndl;
