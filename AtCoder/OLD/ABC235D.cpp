...
// void op2(int a, int n , int i){
// 	string as = to_string(a);
// 	int k = as.length();
// 	string an = to_string(n);
// 	string ns;
// 	if(a>10 && as[k-1] !='0' && a.length() <=n.length()){
// 		ns = as[k-1];
// 		rep(i,0,k-1){
// 			ns.pb(as[i]);
// 		}
...
int32_t count_digit(int number) {
   return int32_t(log10(number) + 1);
}

void op1(int a , int n , int i){
	if( count_digit(a) <=count_digit(n) )
	{
		v.pb({a*aa , i});
	vhash[a*aa] = i;
}
