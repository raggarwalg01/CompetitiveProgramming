...
void solve(){    
      ...
      for(auto &i:v){
       ...
       int ans = 0;
       int sumeven = accumulate(even.begin(),even.end(),0);
       int k = 0;
       for (int i = 0; i < odd.size(); i++,k++)
       {
           ans+=sumeven*odd[i];
           //cout<<odd[i]<<" "<<sumeven<<" "<<even[k]<<" "<<ans<<endl;
           if(k<even.size()){
...
/*
        ...
        umapii preff;
        preff[odd.sz -1] = odd[odd.sz -1];
        repn(i,odd.sz-2,0){
        	preff[i] = preff[i+1] + odd[i];
        }
        // trav(i,preff){
        // 	cerr<< i.se spcend;
        // }
