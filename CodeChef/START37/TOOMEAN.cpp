...
void solve()
{
    ...
    while (val <= n)
    {
        ...
        double temp = pref[n] - pref[n - (val - 1)];
        double rem = n - (val - 1);
        double thiss = pref[rem] / rem;
        double final = (temp + thiss) / val;
        ans = max(ans, final);
        val++;
    }
    ...
    // for (int i = 0; i <= cnt; ++i)
    // {
    //     double avg = 0, newavg = 0, temp = 0;

    //     avg = (1.0 * (sum + i * mx)) / (remcnt + i);
    //     // cout << avg spcend;
    //     if (cnt != i)
    //         temp = (1.0 * (avg + mx)) / (2);
    //     // cout << temp spcend;
    //     // temp = (avg + 1.0 * mx) / (2);
    //     if (cnt - i + 1 > 0)
    //         newavg = (sum + (cnt - i) * mx) / (1 + cnt - i);
    //     ans = max({avg, temp, ans, newavg});
    // if (remcnt + i > 0 and mxleft + 1 > 0)
    // {
    //     avg = (1.0 * sumrr + 1.0 * (mx * i)) / (remcnt + i);
    //     newavg = (avg + 1.0 * (mxleft * mx)) / (mxleft + 1);
    // }
    // // newavg = max( )
    // double temp = 0;
    // if (remcnt + i + 1 > 0)
    // {
    //     temp = (avg + 1.0 * mx) / (remcnt + i + 1);
    // }
    // newavg = max(newavg, temp);
    // ans = max(ans, newavg);
    // mxleft--;
    // cerr << newavg spcend;
    // if (mxleft < 0)
    ...
    //     // if (sum = 0)
    //     avg = (1.0 * sum) / (n - cnt);
    //     double newavg = (avg + 1.0 * mx * cnt) / (cnt + 1);
    //     cout << newavg ndl;
    //     return;
    // }
