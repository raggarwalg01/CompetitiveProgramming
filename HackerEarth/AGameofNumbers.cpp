...
******************************************************************************************/
vector<ll> v;
vector<pair<ll,ll>> nextgrea(ll start,ll end,ll target){
...
ll nextsmall(ll start,ll end,ll target){
   //vector<pair<ll,ll>> res;
    ll res=-1;
    rep(i,start,end){
        if(v[i]<target){
...
void solve(){    
   ...
   rep(i,0,n){
    ...
    temp=nextgrea(i,n,v[i]);
    if(temp.size()==0)
        ans.pb(-1);
    else{
        ll pos2=nextsmall(temp[0].second,n,temp[0].fi);
        if(pos2==-1)
            ans.pb(-1);
        else
            ans.pb(pos2);
